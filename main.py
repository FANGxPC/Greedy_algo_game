...
class GreedyBagRace:
    def __init__(self):
        ...
        # pygame.display.set_caption("Greedy Bag Race")
        ...
        self.ai_move_delay = 60  # Frames between AI moves (2 seconds at 60 FPS)
        ...
        # Initialize backend game engine
    ...
    def init_audio(self):
        ...
        bgm_config = {
            ...
            "AI_WIN": "assets/sounds/lost.mp3",  # This plays when player loses to AI
            "PLAYER1_WIN": "assets/sounds/win.mp3",
            "PLAYER2_WIN": "assets/sounds/win.mp3"  # Fixed path separator
        }
        
        # Debug: Print available sound files
        ...
        try:
            ...
            sound_paths = {
                ...
                'lose': "assets/sounds/lost.mp3"  # Changed from lose.mp3 to lost.mp3 to match the actual file
            }
    ...
    def init_gui(self):
        ...
        # Mode selection buttons (Single/Multiplayer)
        self.mode_buttons = [
    ...
    def run(self):
        while True:
            ...
            self.update()
            self.render()
            self.clock.tick(FPS)  # This ensures 60 FPS and continuous updates
    ...
    def handle_game_click(self, event):
        # Check if human player's bag is full
        ...
        if not self.game_engine.is_ai_turn() and current_player["space_left"] <= 0:
            # Human player's bag is full, skip turn
            self.game_engine.skip_turn()
            # Don't process any other clicks this frame
            return
        ...
        # Only allow item selection if it's human player's turn and bag has space
        if not self.game_engine.is_ai_turn() and current_player["space_left"] > 0:
        ...
        # If human player's bag is full, don't respond to item clicks
    ...
    def update(self):
        ...
        if self.game_state in ["IN_GAME_SINGLE", "IN_GAME_MULTI"]:
            if not self.game_engine.game_over:
                ...
                # If current player's bag is full, skip their turn
                if current.current_weight >= current.bag_limit:
                    self.game_engine.skip_turn()
                # If it's AI's turn, let AI make a move with a delay
                    ...
                    if self.ai_move_timer < self.ai_move_delay:
                        ...
                        # Compare states to find what was picked
                        ...
                        for i, item_before in enumerate(pre_move_state['available_items']):
                            ...
                            if key not in post_items:
                                ...
                                break
                            # Check if item was partially taken
                            else:
    ...
    def render_mode_select(self):
        # title_font = pygame.font.SysFont('arial', self.screen_height // 15, bold=True)
        
        # title_surf = title_font.render("Select Game Mode", True, TEXT_COLOR)
    ...
    def render_difficulty_select(self):
        # title_font = pygame.font.SysFont('arial', self.screen_height // 15, bold=True)
        # font = pygame.font.SysFont('arial', self.screen_height // 30)
        
        # title_surf = title_font.render("Select Difficulty", True, TEXT_COLOR)
        # title_rect = title_surf.get_rect(center=(self.screen_width//2, self.screen_height//4))
        # self.screen.blit(title_surf, title_rect)
        
        # diff_surf = font.render(f"Selected: {self.difficulty}", True, TEXT_COLOR)
        # diff_rect = diff_surf.get_rect(center=(self.screen_width//2, self.screen_height//4 + 80))
    ...
    def render_game_over(self):
        font = pygame.font.SysFont('arial', self.screen_height // 50)  # Smaller font for compact display
        ...
        # Simple score display at bottom center
        ...
        # Draw a small line above the score
        ...
        # Store the back button for click handling
