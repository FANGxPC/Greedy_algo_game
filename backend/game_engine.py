...
class GameEngine:
    def __init__(self, bag_capacity=25):
        ...
        self.ai_players = {}  # player_index -> AI object
    ...
    def _generate_items(self):
        ...
        # Create a list of all possible item numbers (1-50)
        all_item_numbers = list(range(1, 51))
        # Randomly select 25 unique item numbers
        ...
        # Sort items by their number for consistent display
    ...
    def _ai_01_knapsack(self, valid_items, player):
        ...
        for item in sorted_items:
            max_frac = player.get_available_fraction(item)
            if max_frac >= 0.99:  # Can fit the whole item
                return item, 1.0
        # If no whole item fits, take the best partial
        if sorted_items:
            item = sorted_items[0]
            max_frac = player.get_available_fraction(item)
            if max_frac > 0:
    ...
    def _check_game_over(self):
        ...
        # Game ends if all bags are full or no items left
